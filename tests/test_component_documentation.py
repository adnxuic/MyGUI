...
class ComponentDocumentationContractTests(unittest.TestCase):
    ...
    def test_all_27_inspector_profiles_have_dedicated_pages(self):
        """Validate all 27 registered production EditorProfiles have dedicated doc pages."""
        all_expected_keys = {
            (kind, role)
            for kind, roles in ROLES_BY_KIND.items()
            for role in roles
        }
        self.assertEqual(len(all_expected_keys), 27)
        ...
        self.assertEqual(len(seen_pages), 27)
