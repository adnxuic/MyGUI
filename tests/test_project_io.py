...
class ProjectIoTests(unittest.TestCase):
    ...
    def test_only_exact_integer_current_or_migratable_schema_is_accepted(self):
        ...
        self.assertEqual(load_project_file(self.path)["schema_version"], PROJECT_SCHEMA_VERSION)
        schema_v15 = as_schema_v15(valid)
        self.path.write_text(json.dumps(schema_v15), encoding="utf-8")
        self.assertEqual(load_project_file(self.path)["schema_version"], PROJECT_SCHEMA_VERSION)
        schema_v16 = as_schema_v16(valid)
        self.path.write_text(json.dumps(schema_v16), encoding="utf-8")
        self.assertEqual(load_project_file(self.path)["schema_version"], PROJECT_SCHEMA_VERSION)
        ...
        self.assertEqual(load_project_file(self.path)["schema_version"], PROJECT_SCHEMA_VERSION)
        for version in (
            3, 4, 5, 6, 7, 8, 9, 24, 10.0, 11.0, 12.0, 13.0, 14.0,
