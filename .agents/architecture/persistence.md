...
MyGUI saves and validates exact integer schema version 19. Component business
state is the schema-v19 tree; profile selection, Section expansion, QWidget
state, callbacks, typed tree projection keys, and other UI-only data are
excluded. Strictly valid schema-v18 files migrate in memory to v19 by
advancing the version and injecting default `geometry: {"mode": "grid"}`
into each Axes component data while removing the redundant Axes
`properties.in_layout` field; strictly valid schema-v17 files migrate through
v18 to v19; strictly valid schema-v16 files migrate through v17 to v19; strictly
...
schema-v19 snapshot. None of that state is registered or shown before the
...
`mygui.template_library` owns independent strict `mygui-template` schema v3.
...
The template Figure is a schema-v19 component-tree blueprint with template-
...
reconciliation pass before validating the Registry tree and schema-v19
...
and `docs/component-properties-v19.md` in the same change.
