/**
 * The `mygui.qt-lifecycle` scanner (v0.1.0): static, read-only Qt lifecycle
 * and QObject ownership checks over the MyGUI repository. It never modifies
 * the repository, never formats code, never auto-fixes, and never launches
 * the GUI.
 *
 * Rule sources: the Qt lifecycle patterns actually present in the MyGUI
 * codebase (parented vs parentless QTimer construction, stop paths, QThread
 * shutdown vocabulary, class-level disconnect contracts). The scanner does
 * not invent global Qt style conventions.
 */

import { execFile } from 'node:child_process';
import { join } from 'node:path';
import type { ScannerDefinition, ScannerDiagnostic, ScannerFinding, ScannerRequest, ScannerResult } from '../../contracts.ts';
...
function makeSummary(findings: ScannerFinding[]): ScannerResult['summary'] {
  const bySeverity: Partial<Record<ScannerFinding['severity'], number>> = {};
...
export function createQtLifecycleScanner(): ScannerDefinition {
  return {
    ...
    version: '0.1.0',
    description:
      'Static Qt lifecycle and QObject ownership checks for the MyGUI repository: parentless member QTimer without stop path, started QThread without shutdown path, repeatable lambda signal connects without a class-level disconnect.',
