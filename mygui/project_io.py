"""Validate, migrate, save, and load strict schema-v16 project snapshots."""
...
def validate_project_snapshot(snapshot: dict[str, Any]) -> None:
    """Validate one exact current schema-v16 project snapshot."""
...
def project_snapshot(figure_window=None, *, canvas=None) -> dict[str, Any]:
    ...
    figure = normalize_v16_figure(canvas.component_snapshot())
    snapshot = {
...
def load_project_file(filename: str | Path) -> dict[str, Any]:
    ...
    if version == SCHEMA_V13_VERSION:
        return migrate_v15_to_v16(migrate_v14_to_v15(migrate_v13_to_v14(snapshot)))
    if version == SCHEMA_V14_VERSION:
        return migrate_v15_to_v16(migrate_v14_to_v15(snapshot))
    if version == SCHEMA_V15_VERSION:
        return migrate_v15_to_v16(snapshot)
    if version != PROJECT_SCHEMA_VERSION:
        raise ValueError(
            f"Unsupported project schema version {version!r}; only schema "
            f"v{PROJECT_SCHEMA_VERSION}, strict v15 migration, strict v14 "
            "migration, strict v13 migration, and chained strict v10-v12 "
