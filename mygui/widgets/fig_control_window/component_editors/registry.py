...
EDITABLE_DATA_KEYS: dict[EditorKey, frozenset[str]] = {
    ...
    (ComponentKind.FIELD_2D, ComponentRole.HEATMAP): frozenset(
        {"x_ref", "y_ref", "z_ref"}
    ),
    (ComponentKind.FIELD_2D, ComponentRole.CONTOUR): frozenset(
        {"x_ref", "y_ref", "z_ref"}
    ),
...
class EditorRegistry:
    ...
    def validate_production_profiles(self) -> None:
        """Fail fast unless every current kind/role has one exact profile."""
