...
from mygui.application_paths import user_cache_directory, user_log_directory
...
from mygui.database.matlab_fallbacks import (
    _fallback_func_exp,
    _poly_fallback as _poly_fallback,
    _rat_fallback as _rat_fallback,
    _sum_fallback as _sum_fallback,
)
...
def _notify_matlab_state_listeners(enabled: bool) -> None:
    for listener in list(_MATLAB_STATE_LISTENERS):
        try:
            ...
            matlab_logger().exception(
                "MATLAB state listener failed and was detached"
            )
...
def _default_lower_bounds(func_name: str, coefficients: list[str]) -> list[float | None]:
    lower: list[float | None] = [None] * len(coefficients)
...
def _default_upper_bounds(coefficients: list[str]) -> list[float | None]:
...
def _cache_input_paths() -> list[Path]:
    matlab_func_root = _REPO_ROOT / "mygui" / "database" / "matlab_func"
    return [
...
def _run_isolated_process(*args, **kwargs):
    """Indirection used by tests and the bounded child-process runner."""
...
"""
    ...
    try:
        ...
        logger.warning(
            "MATLAB isolated operation exceeded a resource budget op=%s error=%s",
            op,
            exc,
        )
    ...
    _log_subprocess_stderr(logger, op, stderr)
    result_line = None
    ...
    if result_line is None:
        detail = (stderr or stdout or f"exit code {completed.returncode}").strip()
        if detail:
...
def _fit_options_json(fit_options: dict[str, Any] | None) -> str:
    if not fit_options:
        return ""
    fit_options = shared_fit_result.fit_options_for_execution(fit_options)
    fields = (
        "Normalize",
        "Robust",
        "Algorithm",
