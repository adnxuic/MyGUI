...
def _expect_snapshot_object(value: Any, path: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ValueError(f"Invalid table field {path}: expected object.")
    return value


def _expect_snapshot_keys(
    value: dict[str, Any],
    expected: set[str],
    path: str,
) -> None:
    ...
    if actual != expected:
        raise ValueError(
            f"Invalid table field {path}: expected exactly {sorted(expected)}, "
            f"got {sorted(actual)}."
        )


def _expect_snapshot_string(value: Any, path: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Invalid table field {path}: expected string.")
    return value


def _expect_snapshot_int(value: Any, path: str, minimum: int = 0) -> int:
    if type(value) is not int or value < minimum:
        raise ValueError(
            f"Invalid table field {path}: expected integer >= {minimum}."
        )
    return value
...
def _validate_snapshot_value(
    ...
    if column_type == ColumnType.AUTO:
        raise ValueError(f"Invalid table field {path}: AUTO columns may contain only null.")
    if column_type == ColumnType.NUMBER:
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError(f"Invalid table field {path}: expected number or null.")
        if not math.isfinite(float(value)):
            raise ValueError(f"Invalid table field {path}: number must be finite.")
        return
    if column_type == ColumnType.TEXT:
        if not isinstance(value, str):
            raise ValueError(f"Invalid table field {path}: expected string or null.")
        return
    if column_type == ColumnType.BOOLEAN:
        if not isinstance(value, bool):
            raise ValueError(f"Invalid table field {path}: expected boolean or null.")
        return
    if column_type == ColumnType.DATETIME:
        if not isinstance(value, str):
            raise ValueError(
                f"Invalid table field {path}: expected ISO date/time string or null."
            )
        try:
        ...
        return
    raise ValueError(f"Invalid table field {path}: unsupported column type.")
...
class SheetDocument:
    ...
    def from_snapshot(cls, snapshot: dict[str, Any]) -> "SheetDocument":
        ...
        snapshot = _expect_snapshot_object(snapshot, "sheet")
        _expect_snapshot_keys(
            snapshot,
            {"id", "name", "row_count", "columns"},
            "sheet",
        )
        sheet_id = _expect_snapshot_string(snapshot.get("id"), "sheet.id").strip()
        if not sheet_id:
            raise ValueError("Sheet id must not be empty.")
        name = validate_component_name(
            _expect_snapshot_string(snapshot.get("name"), "sheet.name"),
            "Sheet name",
        )
        row_count = _expect_snapshot_int(
        ...
        for column_index, raw_column in enumerate(columns):
            path = f"sheet.columns[{column_index}]"
            column_snapshot = _expect_snapshot_object(raw_column, path)
            _expect_snapshot_keys(
                column_snapshot,
                {"id", "name", "type", "width", "values"},
                path,
            )
            column_id = _expect_snapshot_string(
                column_snapshot.get("id"),
                f"{path}.id",
            ).strip()
            if not column_id:
                raise ValueError("Column id must not be empty.")
            column_name = _expect_snapshot_string(
                column_snapshot.get("name"),
                f"{path}.name",
            )
            raw_type = _expect_snapshot_string(
                column_snapshot.get("type"),
                f"{path}.type",
            )
            try:
                column_type = ColumnType(raw_type)
            except ValueError as exc:
                raise ValueError(
                    f"Invalid table field {path}.type: {raw_type!r}."
                ) from exc
            width = _expect_snapshot_int(
                column_snapshot.get("width"),
                f"{path}.width",
                60,
            )
            raw_values = column_snapshot.get("values")
            if not isinstance(raw_values, list):
                raise ValueError(f"Invalid table field {path}.values: expected array.")
...
class ProjectTableDocument:
    ...
    def from_snapshot(cls, snapshot: dict[str, Any]) -> "ProjectTableDocument":
        ...
        snapshot = _expect_snapshot_object(snapshot, "table")
        _expect_snapshot_keys(snapshot, {"id", "name", "sheets"}, "table")
        project_id = _expect_snapshot_string(snapshot.get("id"), "table.id").strip()
        if not project_id:
            raise ValueError("Project id must not be empty.")
        project_name = validate_component_name(
            _expect_snapshot_string(snapshot.get("name"), "table.name"),
            "Project name",
        )
        project = cls(id=project_id, name=project_name)
        sheets = snapshot.get("sheets")
        if not isinstance(sheets, list):
            raise ValueError("Project sheets must be an array.")
        if not sheets:
            raise ValueError("Project must contain at least one Sheet.")
