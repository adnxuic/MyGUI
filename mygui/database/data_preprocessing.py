...
MAX_PREPROCESS_EXPRESSION_LENGTH = MAX_EXPRESSION_LENGTH
MAX_PREPROCESS_AST_NODES = MAX_EXPRESSION_AST_NODES
MAX_PREPROCESS_AST_DEPTH = MAX_EXPRESSION_AST_DEPTH


def _compile_preprocess_expression(expression: str, axis: str) -> ast.Expression:
    try:
        return compile_math_expression(expression, {"x", "y"})
    except ValueError as exc:
        raise ValueError(
            f"{axis.upper()} preprocessing expression is invalid: {exc}"
        ) from exc
