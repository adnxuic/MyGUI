...
def default_fit_options(fit_type: str) -> dict[str, Any]:
    ...
    options: dict[str, Any] = {
        ...
        "Lower": [
            ...
            for value in spec.default_lower(np.asarray([1.0]), np.asarray([1.0]))
        ],
        "Upper": [
            float(value) if np.isfinite(value) else None
            for value in spec.default_upper(np.asarray([1.0]), np.asarray([1.0]))
        ],
...
def fit_curve(
    ...
    options = fit_options_for_execution(fit_options)
