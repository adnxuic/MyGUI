...
The optional FullProf `.prf` workflow in the **Single Axes** and **Main Plot + Residual** dialogs
...
instead of copying PRF numbers. Main Plot + Residual and Single without residual
use `placement: {"kind": "fixed"}` with `baseline=0.0375` and `height=0.025`
so the marks occupy the center of the 10% lower-Y reserve (`3.75%–6.25%` of
Axes height). Single with **Draw residual** uses
`placement: {"kind": "between_table_ranges"}` so the marks stay centered between
the `Yobs-Ycal (PRF)` maximum and the lowest `Yobs`/`Ycal` values after
ordinary autoscale. Ordinary Reflection Positions keep
...
| Baseline | Number | Lower Y position in normalized Axes coordinates. Automatic `between_table_ranges` placement keeps this value read-only until **Convert to fixed position**. | Finite `0 <= baseline <= 1`; default `0.08` | `properties.baseline` |
...
| Placement sources | Read-only labels plus **Convert to fixed position** | Shows fixed placement or the three Table columns that bound automatic vertical placement. Convert stores the current baseline and height as `{"kind": "fixed"}`. | `fixed` or `between_table_ranges` with `lower_ref` and two `upper_refs`; default `fixed` | `data.placement` |
...
`data` object contains exactly `positions`, nullable `position_ref`, and tagged
`placement`; the
