...
| Major Locator | Locator dialog | Rule determining where major tick marks are placed along the X axis. | Locator spec; default `{'kind': 'auto'}` | `properties.major_locator` |
...
Schema v15 persists the X Axis as `kind: "axis"`, `role: "x_axis"`, with `selector: {"axis_name": "x"}` under its parent Axes.
