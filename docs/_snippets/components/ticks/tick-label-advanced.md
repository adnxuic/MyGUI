...
| Transform Rotates Text | Checkbox | Determines whether coordinate transformations rotate label geometry. | `true` or `false`; default `false` | `properties.transform_rotates_text` |
| Clip On | Checkbox | Clips tick labels to the parent Axes boundary. | `true` or `false`; default `true` | `properties.clip_on` |
| Gid | Text | SVG group identifier for vector export. | String or none; default none | `properties.gid` |
| In Layout | Checkbox | Includes labels in layout calculations. | `true` or `false`; default `true` | `properties.in_layout` |
