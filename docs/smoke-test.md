...
5. Open Settings (gear, **Edit > Settings**, or Ctrl+,). Confirm the page
   ...
   preview. On Components, Line / Scatter / Text are separate tabs; toggle a
   Line width override, Cancel, reopen the
   cached window, and confirm inherit is restored. Apply an override, create
   a Curve, confirm the new width, then confirm an existing curve is
   unchanged. On Axes Components, override Facecolor, Apply, confirm an
   existing Axes is unchanged, then create a new Axes on a new Figure and
   confirm the new face. Restore page defaults and Reset all both return inherit
   and do
   not clear the color library. Restart and confirm the last Apply persisted.
...
8. In Settings → Components, override Line width and Scatter marker, Apply,
   then create a Curve and a Scatter. Confirm only new components use those
   values. Reopen the project and confirm the saved properties did not pick
   up later Settings changes. Confirm a failed or cancelled creation does not
   advance the palette or write recents.
9. In Settings → Axes Components, override Facecolor and a spine color, Apply,
   confirm an already created Axes is unchanged, then create a main Axes on a
   new Figure, a right-Y pair, and an XRD Axes. Confirm new Axes pick up the
   defaults,
   explicit dialog/XRD values still win, and Cancel in Settings discards the
   draft. Reopen Settings and confirm Apply persisted.
