# Component Properties (schema v19)
...
Schema v19 retains the exact eight-field `ComponentState` record:
...
This coordination changes no schema-v19 field or record shape.
...
Schema v19 extends schema v18 by introducing the required per-Axes `geometry`
data field (`{"mode": "grid"}` or `{"mode": "manual", "bounds": [left, bottom, width, height]}`).
When saving, projects record `schema_version: 19`. Existing strictly valid
schema-v18 files migrate directly into schema v19 by populating `geometry: {"mode": "grid"}`
for all Axes components and removing the redundant Axes `properties.in_layout`
field. Stepwise migrations from v10–v17 promote into schema v18
and then directly into v19. Prior schema versions (v10–v18) reject `geometry` in Axes data.
